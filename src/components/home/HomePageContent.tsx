"use client";

import Link from "next/link";
import { Heart, Share2, QrCode, Star, ChevronRight } from "lucide-react";
import AnimatedMockup from "@/components/home/AnimatedMockup";
import ScrollReveal from "@/components/home/ScrollReveal";

type Props = {
    isLoggedIn: boolean;
};

export default function HomePageContent({ isLoggedIn }: Props) {
    return (
        <div className="flex flex-col min-h-screen bg-black text-white font-sans">
            {/* Top banner only — toned down (rest of page stays vibrant pink) */}
            <div className="relative z-30 bg-zinc-950/95 border-b border-white/10 py-2.5 text-center text-sm backdrop-blur-md">
                <p className="text-gray-300 font-medium tracking-wide">
                    <span className="text-pink-heart mr-1.5">✨</span>
                    Premium love pages —{" "}
                    <span className="text-white font-semibold">50% off</span> today
                </p>
            </div>

            <section className="relative pt-12 sm:pt-20 pb-20 sm:pb-32 overflow-hidden">
                <div className="absolute inset-0 z-0 pointer-events-none">
                    <div className="absolute top-20 left-10 text-pink-heart/90 animate-bounce delay-100">
                        <Heart size={40} className="fill-current" />
                    </div>
                    <div className="absolute top-40 right-20 text-pink-heart/80 animate-pulse">
                        <Heart size={60} className="fill-current" />
                    </div>
                    <div className="absolute bottom-20 left-1/4 text-pink-heart/70 animate-bounce delay-700">
                        <Heart size={30} className="fill-current" />
                    </div>
                </div>

                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                    <div className="flex flex-col lg:flex-row items-center gap-12 text-center lg:text-left">
                        <ScrollReveal className="flex-1 space-y-8" direction="left">
                            <div className="inline-flex items-center px-4 py-2 rounded-full border border-red-primary/30 bg-red-primary/10 text-red-primary text-sm backdrop-blur-sm shadow-[0_0_15px_rgba(255,0,51,0.3)]">
                                <Heart className="w-4 h-4 mr-2 text-pink-heart fill-current" />
                                Want to start?
                            </div>

                            <h1 className="text-4xl sm:text-5xl md:text-7xl font-bold tracking-tight leading-tight selection:bg-red-500 selection:text-white">
                                Declare your love <br />
                                <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#ff4d4d] to-[#ffccd5] drop-shadow-sm">
                                    for someone special!
                                </span>
                            </h1>

                            <p className="text-xl text-gray-400 max-w-2xl mx-auto lg:mx-0 leading-relaxed">
                                Create a personalized page for who you love and surprise that
                                person with a special declaration that will last forever.
                            </p>

                            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start pt-4">
                                <Link
                                    href={isLoggedIn ? "/dashboard" : "/register"}
                                    className="inline-flex items-center justify-center px-8 py-4 text-lg font-bold rounded-lg text-white bg-button-gradient hover:opacity-90 shadow-lg shadow-red-900/50 transition-all transform hover:scale-105"
                                >
                                    <Heart className="w-5 h-5 mr-3 heart-white fill-current" />
                                    {isLoggedIn ? "Go to Dashboard" : "Create my page"}
                                    <ChevronRight className="w-5 h-5 ml-2" />
                                </Link>
                            </div>

                            <div className="bg-background-card/50 backdrop-blur-md p-4 rounded-xl border border-white/10 inline-block text-left mt-8 max-w-sm">
                                <div className="flex text-yellow-400 mb-1">
                                    {[0, 1, 2, 3, 4].map((i) => (
                                        <Star key={i} className="w-4 h-4 fill-current" />
                                    ))}
                                </div>
                                <p className="text-sm font-medium text-gray-300">
                                    Loved by couples worldwide
                                </p>
                                <p className="text-xs text-gray-500 mt-1">
                                    Join 10,000+ happy couples
                                </p>
                            </div>
                        </ScrollReveal>

                        <ScrollReveal
                            className="flex-1 relative flex justify-center lg:justify-end"
                            direction="right"
                            delay={0.15}
                        >
                            <div className="absolute -left-10 top-1/4 bg-gradient-to-r from-pink-heart/20 to-transparent border border-pink-heart/30 px-4 py-2.5 rounded-xl text-xs text-white z-20 shadow-lg shadow-pink-heart/10 hidden md:block backdrop-blur-sm">
                                <span className="text-pink-heart">♥</span> Live love pages
                            </div>
                            <div className="absolute -right-5 top-1/3 bg-gradient-to-l from-pink-heart/20 to-transparent border border-pink-heart/30 px-4 py-2.5 rounded-xl text-xs text-white z-20 shadow-lg shadow-pink-heart/10 hidden md:block backdrop-blur-sm">
                                Animated & beautiful{" "}
                                <span className="text-pink-heart">♥</span>
                            </div>
                            <div className="absolute -right-2 bottom-1/4 bg-background-card/80 border border-white/10 px-3 py-2 rounded-lg text-[10px] text-gray-300 z-20 hidden md:block backdrop-blur-md">
                                Our relationship — couple love pages
                            </div>
                            <AnimatedMockup />
                        </ScrollReveal>
                    </div>
                </div>
            </section>

            <section
                id="how-it-works"
                className="py-24 bg-background-card border-t border-white/5 relative overflow-hidden"
            >
                <div
                    className="absolute inset-0 pointer-events-none opacity-40"
                    style={{
                        background:
                            "radial-gradient(ellipse 70% 50% at 50% 100%, rgba(255,107,157,0.12) 0%, transparent 60%)",
                    }}
                />
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                    <ScrollReveal className="text-center mb-16">
                        <h2 className="text-red-primary font-bold tracking-wide uppercase text-sm mb-2">
                            How it works
                        </h2>
                        <h2 className="text-3xl md:text-4xl font-bold text-white">
                            Create your surprise in 3 steps
                        </h2>
                    </ScrollReveal>

                    <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                        <FeatureCard
                            delay={0.1}
                            icon={<Star className="h-8 w-8 text-red-primary" />}
                            title="1. Customize"
                            description="Choose your colors, photos, and write a message from your heart."
                        />
                        <FeatureCard
                            delay={0.25}
                            icon={<QrCode className="h-8 w-8 text-red-primary" />}
                            title="2. Generate QR"
                            description="Get a unique QR code automatically created for your page."
                        />
                        <FeatureCard
                            delay={0.4}
                            icon={<Share2 className="h-8 w-8 text-red-primary" />}
                            title="3. Surprise!"
                            description="Share the link or print the QR code to surprise your love."
                        />
                    </div>
                </div>
            </section>

            <ScrollReveal>
                <footer className="py-8 bg-black text-center text-gray-600 text-sm border-t border-white/10">
                    <p>© {new Date().getFullYear()} LoveyLink. All rights reserved.</p>
                    <div className="mt-4">
                        <Link
                            href="/login"
                            className="text-gray-700 hover:text-gray-500 transition-colors text-xs"
                        >
                            Admin Login
                        </Link>
                    </div>
                </footer>
            </ScrollReveal>
        </div>
    );
}

function FeatureCard({
    icon,
    title,
    description,
    delay = 0,
}: {
    icon: React.ReactNode;
    title: string;
    description: string;
    delay?: number;
}) {
    return (
        <ScrollReveal delay={delay}>
            <div className="p-8 bg-black/50 rounded-2xl border border-white/10 hover:border-red-primary/50 hover:shadow-lg hover:shadow-pink-heart/10 transition-all duration-300 group h-full">
                <div className="mb-6 p-4 bg-red-primary/10 rounded-xl inline-block group-hover:bg-red-primary/20 transition-colors">
                    {icon}
                </div>
                <h3 className="text-xl font-bold text-white mb-3">{title}</h3>
                <p className="text-gray-400 leading-relaxed">{description}</p>
            </div>
        </ScrollReveal>
    );
}
